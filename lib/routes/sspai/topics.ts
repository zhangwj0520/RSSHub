    ...
    const items = await Promise.all(
        data.map((item) => {
            ...
            return cache.tryGet(key, () => {
                description = `<br><img src="https://cdnfile.sspai.com/${item.banner}" alt="Article Cover Image" style="display: block; margin: 0 auto;"/>${item.intro}<br>如有兴趣,请复制链接订阅 <br> <h3>https://rss.windego.cn/sspai/topic/${item.id}</h3>`;
